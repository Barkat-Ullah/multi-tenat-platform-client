...
const ShoppingCartIcon = ({ className = "w-10 h-10" }: { className?: string }) => (
  <svg xmlns="http://www.w3.org/2000/svg" width="38" height="39" viewBox="0 0 38 39" fill="none" className={className}>
    <path d="M4.60435 20.4389H26.6306L29.3233 6.59277H2.39634C2.19943 6.59352 2.00507 6.63867 1.82697 6.72506C1.64887 6.81144 1.49136 6.93695 1.36555 7.09274C1.23974 7.24854 1.14869 7.43081 1.09882 7.62672C1.04895 7.82263 1.04148 8.02741 1.07692 8.22661L3.28493 19.3035C3.33644 19.6238 3.49788 19.9144 3.73996 20.1227C3.98205 20.3311 4.28873 20.4432 4.60435 20.4389V20.4389Z" stroke="#D2B48C" strokeWidth="2.10972" strokeLinecap="round" strokeLinejoin="round"/>
    <path d="M29.3169 6.59312L30.4478 2.16237C30.5099 1.84963 30.6751 1.56854 30.9156 1.36669C31.156 1.16484 31.4569 1.05462 31.7672 1.05469H36.0486" stroke="#D2B48C" strokeWidth="2.10972" strokeLinecap="round" strokeLinejoin="round"/>
    <path d="M26.6213 20.4385L25.4904 26.2538C25.4283 26.5666 25.2631 26.8477 25.0226 27.0495C24.7822 27.2514 24.4813 27.3616 24.171 27.3615H7.77246" stroke="#D2B48C" strokeWidth="2.10972" strokeLinecap="round" strokeLinejoin="round"/>
    <path d="M9.11881 37.0554C9.86237 37.0554 10.4652 36.4354 10.4652 35.6707C10.4652 34.906 9.86237 34.2861 9.11881 34.2861C8.37524 34.2861 7.77246 34.906 7.77246 35.6707C7.77246 36.4354 8.37524 37.0554 9.11881 37.0554Z" stroke="#D2B48C" strokeWidth="2.10972" strokeLinecap="round" strokeLinejoin="round"/>
    <path d="M22.5832 37.0554C23.3267 37.0554 23.9295 36.4354 23.9295 35.6707C23.9295 34.906 23.3267 34.2861 22.5832 34.2861C21.8396 34.2861 21.2368 34.906 21.2368 35.6707C21.2368 36.4354 21.8396 37.0554 22.5832 37.0554Z" stroke="#D2B48C" strokeWidth="2.10972" strokeLinecap="round" strokeLinejoin="round"/>
  </svg>
);
...
const StepIndicator: React.FC<{ steps: number[] }> = ({ steps }) => {
  const middleIndex = Math.floor(steps.length / 2); // Find middle index
    ...
    <div className="relative flex items-center justify-between w-full max-w-2xl mx-auto mb-12">
      ...
      <div className="relative z-10 flex justify-between w-full">
        {steps.map((num, index) => (
          <div 
            key={num} 
            className={`flex items-center justify-center w-12 h-12 rounded-full font-bold text-xl shadow-sm ${
              index === middleIndex 
                ? 'bg-[#ceffd3de] text-green-600' // ✅ Middle step - bright green
                : 'bg-[#deecf1] text-[#004E60]' // Other steps - original colors
            }`}
          >
            {num}
          </div>
...
const SimpleCard: React.FC<{ title: string; description: string; icon: React.ReactNode }> = ({ 
  ...
  <div className="relative pt-10">
    ...
    <div className="bg-white rounded-2xl shadow-md pt-14 pb-8 px-8 flex flex-col items-center text-center hover:shadow-lg transition-shadow">
...
const ImageCard: React.FC<FocusCardData> = ({ title, description, image, type }) => (
  <div className="relative border border-[#D4D4D4] px-3 md:pb-3 pb-3 sm:pb-0 pt-20 rounded-3xl">
    {/* Icon overlapping top */}
    <div className="absolute top-9 left-1/2 -translate-x-1/2 w-20 h-20 rounded-full bg-white shadow-lg border border-gray-100 flex items-center justify-center z-10">
      {type === 'rent' ? (
