...
import type { MotorhomeIntroData } from "@/app/data/OtherMedicalData";

interface MotorhomeIntroSectionProps {
  data: MotorhomeIntroData;
}

export default function MotorhomeIntroSection({ data }: MotorhomeIntroSectionProps) {
  return (
    <section className="bg-white py-16 sm:py-20 lg:py-24 poppins relative overflow-hidden border-b border-slate-100">
      <div className="mx-auto max-w-[1440px] px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <div className="flex flex-col lg:flex-row lg:justify-between items-center gap-12 lg:gap-16">
          ...
          <div className="w-full lg:w-[42%] flex justify-center lg:justify-start">
          ...
          <div className="w-full lg:w-[50%] flex flex-col justify-center">
            ...
            <div className="space-y-6 mb-8">
              ...
              {/* What to Bring Checklist with inline links */}
                ...
                <ul className="space-y-2.5">
                  {data.whatToBringItems.map((item, index) => (
                    <li key={index} className="flex items-start gap-3">
                      ...
                      <svg
                        className="w-5 h-5 text-[#00B2D6] stroke-[2.5] flex-shrink-0 mt-0.5"
                        fill="none"
                        viewBox="0 0 24 24"
                        stroke="currentColor"
                      >
                          ...
                          if (seg.isLink && seg.linkUrl) {
                            return (
                              <Link
                                key={sIdx}
                                href={seg.linkUrl}
                                className="text-[#00B2D6] hover:text-[#0092B3] underline underline-offset-2 transition-colors font-bold"
                              >
                                {seg.text}
                              </Link>
                            );
