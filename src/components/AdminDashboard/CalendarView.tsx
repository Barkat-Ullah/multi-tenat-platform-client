...
import { adminCalendarAppointments, CalendarAppointmentData } from "@/app/data/AdminDashboardData";
...
export default function CalendarView() {
  ...
  const [appointments] = useState<CalendarAppointmentData[]>(adminCalendarAppointments);
  ...
  const monthCellRender = (current: Dayjs, info: { type: string }) => {
    if (info.type === "date") {
      // Map Dayjs day of week index to day keys (MON, TUE...)
      ...
      // Popover layout listing appointments beautifully
  ...
  return (
    <div className="p-4 md:p-6 lg:p-8 space-y-6">
      ...
      {/* Render Weekly View Grid (matching mockup grid) */}
      {viewMode === "Weekly" && (
        <div className="overflow-x-auto rounded-3xl border border-slate-100 shadow-[0_4px_25px_rgba(0,0,0,0.015)] bg-white">
          <table className="w-full border-collapse min-w-[900px]">
            <thead>
              <tr className="bg-slate-50/70 border-b border-slate-100">
                ...
                <th className="py-4 px-4 text-xs font-bold text-[#00B2D6] uppercase tracking-wider text-center border-r border-slate-100 bg-[#E6FAFF]/60 w-[13%]">
                  <div>MON</div>
                  <div className="text-[10px] text-[#00B2D6] font-bold mt-0.5">TODAY</div>
                </th>
                <th className="py-4 px-4 text-xs font-bold text-slate-500 uppercase tracking-wider text-center border-r border-slate-100 w-[13%]">
                  TUE
                </th>
                <th className="py-4 px-4 text-xs font-bold text-slate-500 uppercase tracking-wider text-center border-r border-slate-100 w-[13%]">
                  WED
                ...
                <th className="py-4 px-4 text-xs font-bold text-slate-500 uppercase tracking-wider text-center w-[13%]">
                  SUN
                </th>
              </tr>
            </thead>
                        ...
                        <div className="flex flex-col gap-1.5 min-h-[50px] justify-center">
                          {cellApps.map((app) => (
                            <div
                              key={app.id}
                              onClick={() => handlePillClick(app)}
                              className={`rounded-lg px-2.5 py-1.5 text-[10px] sm:text-[11px] font-bold tracking-wide text-center text-white cursor-pointer transition-all hover:scale-[1.02] active:scale-[0.99] shadow-sm select-none ${
                                app.color === "cyan"
      ...
      <style jsx global>{`
        ...
        .premium-antd-calendar .ant-picker-calendar-date-today {
          ...
          background-color: #e6faff/30 !important;
        }
