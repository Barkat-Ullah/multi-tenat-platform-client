...
import { Table, Button, Input, Menu, Modal, List, Empty, Skeleton, Spin, Tooltip, Dropdown, message } from "antd";
...
import { useDeletePropertyMutation, useGetAgencyPropertiesQuery, useGetPropertyMessagesQuery, useUpdatePropertyStatusMutation } from "@/redux/service/agent/propertiesApi";
  ...
  const handleDeleteProperty = async (propertyId: string) => {
    try {
      ...
      appAlert.fire({
        icon: 'error',
        title: 'Error',
        text: error?.data?.message || error?.message || 'Failed to delete property'
      });
    }
  };
  ...
  const columns = [
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="w-16 h-16 bg-gray-300 rounded-lg overflow-hidden flex-shrink-0">
            <Image
              width={64}
              height={64}
              src={getImageUrl(mappedProperties.image)}
              alt="Property"
          ...
          <div>
            <div className="font-semibold text-base text-[#2B2B2B] font-poppins mb-1">
              <Tooltip title={mappedProperties.name}>
                <span
                  style={{
    ...
    },
    {
      ...
      render: (status: string) => (
        <div className="m-0 border text-base font-semibold font-robotomono text-[#F8F8F6] bg-[#D2B48C] p-1 px-4 mx-auto rounded-lg flex items-center justify-center gap-2">
          {status}
        </div>
      ),
    },
    ...
    },
    {
      ...
      render: (status: string, record: any) => {
        const handleStatusClick = async (info: any) => {
          try {
            await updatePropertyStatus({ id: record.id, status: info.key }).unwrap();
            message.success(`Status updated to ${info.key}`);
          } catch (err: any) {
        ...
        return (
          <div className="text-xs">
            <Dropdown
              ...
              trigger={["click"]}
            >
              <div className="m-0 border text-base font-normal font-robotomono text-white bg-[#3BB273] p-1 px-4 rounded flex items-center justify-center gap-2 cursor-pointer">
      ...
      render: (_: any, record: any) => {
        ...
        `}
            >
              ...
              {/* Edit Button - Hidden on Desktop per user request */}
    ...
    <div className="flex flex-col overflow-hidden bg-gray-50" style={{ height: "calc(100vh - 64px)" }}>
      ...
      <div className="flex lg:hidden flex-col h-full overflow-hidden">
        ...
        <div className="bg-white border-b border-gray-200 px-4 py-3 flex-shrink-0 flex items-center gap-2">
          ...
          />
          <Button href="/dashboard/user/property-list/add-property" type="primary" size="middle" className="bg-[#004E60] flex-shrink-0">
            + Add
          </Button>
        </div>
            ...
            <>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                {mappedProperties.map((record: any) => {
                  const handleCardStatusClick = async (info: any) => {
                    try {
                      await updatePropertyStatus({ id: record.id, status: info.key }).unwrap();
                      message.success(`Status updated to ${info.key}`);
                    } catch (err: any) {
                    ...
                    <div key={record.id} className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden hover:shadow-md transition-shadow">
                      <div className="relative w-full h-40 bg-gray-200">
                        ...
                        <div className="absolute top-2 left-2 flex gap-1">
                          <button 
                            onClick={(e) => { e.stopPropagation(); handleDeleteProperty(record.id); }}
                            className="bg-red-500 text-white p-1.5 rounded-full shadow hover:bg-red-600 transition-colors flex items-center justify-center cursor-pointer"
                            title="Delete Property"
                          >
                            <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M3 6h18"/><path d="M19 6v14c0 1-1 2-2 2H7c-1 0-2-1-2-2V6"/><path d="M8 6V4c0-1 1-2 2-2h4c1 0 2 1 2 2v2"/><line x1="10" y1="11" x2="10" y2="17"/><line x1="14" y1="11" x2="14" y2="17"/></svg>
                          </button>
                        </div>
                        <Dropdown menu={{ items: statusItems, onClick: handleCardStatusClick }} trigger={["click"]}>
                          <button className="absolute top-2 right-2 bg-[#3BB273] text-white text-xs font-poppins px-2 py-0.5 rounded-full flex items-center gap-1 cursor-pointer shadow">
                            {record.status}
                            <svg width="10" height="10" viewBox="0 0 24 24" fill="none"><path d="M6 9L12 15L18 9" stroke="white" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" /></svg>
                          </button>
                        </Dropdown>
                      </div>
                      <div className="p-3">
                        ...
                        <div className="flex items-center gap-2 pt-2 border-t border-gray-100">
                          ...
                          <Link href={`/dashboard/user/property-list/edit-property/${record.id}`} className="flex-1"><button className="w-full text-xs py-1.5 px-2 rounded border border-[#004E60] text-[#004E60] font-poppins hover:bg-[#004E60] hover:text-white transition-colors">Edit</button></Link>
      ...
      <div className="hidden lg:flex flex-1 overflow-hidden">
        {/* Main Content Area - Flexible with proper overflow */}
        <div className="flex flex-1 overflow-hidden">
          ...
          <div className="w-56 bg-white border-r border-gray-200 flex-shrink-0 overflow-y-auto">
            <div className="p-6">
              {/* Add Property Button - Hidden on Desktop per user request */}
              {/* <Button
              type="primary"
              icon={
                <svg
